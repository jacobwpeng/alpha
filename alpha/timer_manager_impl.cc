...
namespace alpha {
    class TimerManager::Impl {
        public:
            ...
            TimerId AddPeriodicalTimer(alpha::TimeStamp expire_time, 
                    uint32_t interval_ms, TimerFunctor functor);
    ...
    TimerManager::Impl::Timer::Timer(
            TimerManager::TimerId id, TimeStamp expire_time, 
    ...
    bool TimerManager::Impl::TimerEntryCompare::operator()(
    ...
    TimerManager::TimerId TimerManager::Impl::AddTimer(
            alpha::TimeStamp expire_time, TimerFunctor functor) {
    ...
    void TimerManager::Impl::RemoveTimer(TimerId id) {
        ...
        TimerEntry entry = std::make_pair(active->second->expire_time_, 
                active->second.get());
    ...
    TimerManager::TimerFunctorList TimerManager::Impl::Step(alpha::TimeStamp now) {
        ...
        Timer* max_timer_ptr = reinterpret_cast<Timer*>(
    ...
    TimerManager::Impl::Timer* TimerManager::Impl::Insert(alpha::TimeStamp expire_time,
            TimerFunctor f) {
        TimerId id = ++current_;
    ...
    bool TimerManager::Expired(TimerId id) const {
        return impl_->Expired(id);
    }
}
