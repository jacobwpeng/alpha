...
#define MemoryListType                                             \
  MemoryList<T, typename std::enable_if < std::is_pod<T>::value && \
                    !std::is_pointer<T>::value>::type >

template <typename T>
std::unique_ptr<MemoryListType> MemoryListType::Create(char* buffer,
                                                       SizeType size) {
...
template <typename T>
std::unique_ptr<MemoryListType> MemoryListType::Restore(char* buffer,
                                                        SizeType size) {
  if (size < sizeof(Header)) {
    return nullptr;
  }
  auto header = reinterpret_cast<Header*>(buffer);
  if (header->magic != kMagic || header->buffer_size < size ||
