...
namespace alpha {
    ...
    LogMessage::LogMessage(const char* file, int line, LogLevel level,
        ...
        :flushed_(false), file_(file), line_(line), level_(level),
        errno_message_(errno_message), expr_(expr) {
        ...
        auto nbytes = snprintf(log_buffer,
                    kLogBufferSize,
                    time_format,
                    tv.tv_usec,
                    tid,
        ...
        stream_.rdbuf()->pubsetbuf(log_buffer + nbytes,
    ...
    alpha::LoggerStream& LogMessage::stream() {
        return stream_;
    }
