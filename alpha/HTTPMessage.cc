...
HTTPHeaders& HTTPMessage::Headers() { return headers_; }

const HTTPHeaders& HTTPMessage::Headers() const { return headers_; }
...
void HTTPMessage::SetStatus(uint16_t status) { response().status = status; }
...
uint16_t HTTPMessage::ClientPort() const { return request().client_port; }

std::string HTTPMessage::Method() const { return request().method; }

std::string HTTPMessage::Path() const { return request().path; }

std::string HTTPMessage::QueryString() const { return request().query_string; }

uint16_t HTTPMessage::Status() const { return response().status; }

const std::string& HTTPMessage::StatusString() const {
  return response().status_string;
}

const std::string& HTTPMessage::Body() const { return body_; }
