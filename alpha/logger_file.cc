...
void LoggerFile::MaybeChangeLogFile() {
  ...
  char file[PATH_MAX];
  ::snprintf(file, sizeof(file), "%s/%s.%4d%02d%02d%02d.%s.log", path_.data(),
             prog_name_.data(), result.tm_year + 1900, result.tm_mon + 1,
             result.tm_mday, result.tm_hour, log_level_name_.data());
...
std::string LoggerFile::SymLinkPath() const {
  char file[PATH_MAX];
  ::snprintf(file, sizeof(file), "%s/%s.%s.log", path_.data(),
             prog_name_.data(), log_level_name_.data());
  return file;
}
}
