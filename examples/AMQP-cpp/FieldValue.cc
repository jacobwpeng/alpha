...
#define FieldValueAsPtr(C, cpp_type, type_enum)                               \
  template <>                                                                 \
  typename std::add_pointer<C() cpp_type>::type FieldValue::AsPtr<cpp_type>() \
      C() {                                                                   \
    CHECK(type_ == type_enum) << "Expected type: `" << type_enum              \
                              << "', Actual type: `" << type_ << "'";         \
    return reinterpret_cast<C() typename std::add_pointer<cpp_type>::type>(   \
        custom.ptr);                                                          \
  }
...
FieldValue::FieldValue(FieldValue::Type string_type, alpha::Slice arg) {
  CHECK(string_type == Type::kShortString || string_type == Type::kLongString)
...
FieldValue::FieldValue(const FieldValue& other) : type_(Type::kEmpty) {
  *this = other;
}
...
FieldValue::Type FieldValue::type() const { return type_; }
