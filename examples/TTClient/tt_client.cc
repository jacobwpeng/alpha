...
void Client::SetCoroutine(alpha::Coroutine* co) { co_ = co; }
...
bool Client::Connected() const { return state_ == ConnectionState::kConnected; }
...
bool Client::Write(const uint8_t* buffer, int size) {
  return conn_->Write(
      alpha::Slice(reinterpret_cast<const char*>(buffer), size));
}

alpha::Slice Client::Read() { return conn_->ReadBuffer()->Read(); }

Iterator::Iterator(Client* client) : client_(client) {}

std::string Iterator::key() const { return key_; }

int Iterator::status() const { return status_; }

void Iterator::Next() { client_->Next(this); }
}
