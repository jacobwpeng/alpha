...
KeyValuePairDecodeUnit::KeyValuePairDecodeUnit(std::string* key,
                                               std::string* val)
    : key_(key), val_(val) {}
...
RawDataEncodedUnit::RawDataEncodedUnit(alpha::Slice data) : data_(data) {
  assert(!data_.empty());
}
...
KeyValuePairEncodeUnit::KeyValuePairEncodeUnit(alpha::Slice key,
                                               alpha::Slice val)
    : key_(key), val_(val) {}
...
CodecStatus ProtocolCodec::Decode(int* consumed) {
  ...
  if (!decode_units_.empty() &&
      current_decode_unit_index_ < decode_units_.size()) {
    for (size_t idx = current_decode_unit_index_; idx < decode_units_.size();
         ++idx) {
      int nbytes = 0;
      auto unit = decode_units_[idx];
...
int ProtocolCodec::ConsumedBytes() const { return consumed_; }

void ProtocolCodec::ClearConsumed() { consumed_ = 0; }

int16_t ProtocolCodec::magic() const { return magic_; }
...
bool ProtocolCodec::Encode() {
  ...
  for (auto idx = current_encode_unit_index_; idx < encode_units_.size();
       ++idx) {
...
size_t ProtocolCodec::MaxBytesCanWrite() const { return l_(); }
...
void ProtocolCodec::SetNoReply() { no_reply_ = true; }

bool ProtocolCodec::NoReply() const { return no_reply_; }

int8_t ProtocolCodec::err() const { return err_; }
}
